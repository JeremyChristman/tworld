...
gcov, unioned across the C and C++ builds. **The unit layer, plus the Qt layer for the `oshw-qt/`
rows** — nothing else reaches those, and the Qt run instruments `oshw-qt/` and nothing else, so every
other row still means exactly what it always did. The end-to-end tests drive an uninstrumented
executable, so what they reach is not counted and these figures understate the suite.

⚠ **A `-CheckBaseline` run that could not MEASURE a baseline row now fails**, naming it. It used to
walk the measured rows and skip anything absent from the baseline — correct for a newly added file,
and blind in reverse: the `oshw-qt/` rows exist only when the Qt layer ran, so on a machine without
Qt5 they were compared against nothing and reported success. Absent is not the same as uncovered.

🔴 **AND THE UNION WAS LOSING DATA UNTIL 2026-09-14.** gcov names its output after the SOURCE file,
so two translation units built from one source wrote the same `.gcov.json.gz` and the second
overwrote the first. `CCMetaData.cpp` read 8.8% against a test written specifically for it; alone it
reports 92.1%. Each gcov run is now drained before the next starts. The bug predated the Qt rows —
the C and C++ builds of any one unit test collide the same way — and it only ever moved numbers
DOWN, which is why a floor-shaped gate never caught it.
