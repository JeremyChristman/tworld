/* fuzz_lxlogic.c: libFuzzer target for the LYNX ENGINE, not just the parser.
 *
 * MOD (Jeremy).
 *
 *     test/run-fuzz.sh lxlogic
 *
 ...
 * `creaturearray` here, where the MS engine keeps three separate lists -- so
 * shutdown() is called on every path out, and the PRNG is reseeded to a FIXED
 * value each run. Without both, a crash would depend on the inputs that came
 * before it and the reproducer would not reproduce.
 *
 * ⚠ AND THE LYNX ENGINE IS WHERE THIS MATTERS MOST, because 909 of the
 * maintainer's recorded solutions are Lynx. A crash here is reachable by
 * anybody who downloads a level pack and presses tab.
 *
 * ⚠ AN _assert FAILURE IS A FINDING, NOT NOISE. lxlogic.c's _assert calls
...
/* NOTE: no `int pedanticmode` here -- lxlogic.c:54 defines it, and defining it
 * again is a redefinition error. The MS target must; this one must not. */
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    fuzzstate.ruleset = Ruleset_Lynx;
    fuzzstate.replay = -1;
